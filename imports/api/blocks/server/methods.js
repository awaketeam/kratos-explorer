...
Meteor.methods({
    ...
    'blocks.blocksUpdate': function() {
        ...
        if (until > curr) {
            ...
            try{
                response = HTTP.get(url);
                let unbonding_list = JSON.parse(response.content);
                JSON.parse(response.content).result.forEach((validator) => validatorSet[validator.consensus_pubkey] = validator)
            }
            catch(e){
                console.log(url);
            ...
            // console.log(JSON.stringify(validatorSet));
            ...
            for (let height = curr+1 ; height <= until ; height++) {
                ...
                try{
                    ...
                    if (response.statusCode == 200){
                            ...
                            for (t in block.block.data.txs){
                                Meteor.call('Transactions.index', sha256(Buffer.from(block.block.data.txs[t], 'base64')), blockData.time, (err, result) => {
                                    if (err){
                                        console.log(block);
                                        console.log(err);
                                        throw Error('when insert action---------->')
                                    }
                                });
                            }
                            // throw Error('block.block.data.txs')
                        }
                        ...
                        if (validators.result){
                            ...
                            for (v in validators.result.validators){
                                ...
                                if (!valExist){
                                    ...
                                    validator.operator_address = getAddress(validator.pub_key);
                                    ...
                                    if (validatorData){
                                        ...
                                        validator.operator_address = validator.address || validatorData.operator_address;
                                        validator.operator_account = validatorData.operator_account;
                                        validator.delegator_address = validator.address || Meteor.call('getDelegator', validator.operator_pubkey);
                                        validator.jailed = ('jailed' in validatorData && typeof validatorData.jailed == 'boolean' ) ? validatorData.jailed : false;
                                        let status_dict = {
                                          '1': 'unbonding',
                                          '2': 'unbonded',
                                          '3': 'bonded',
                                        }
                                        // throw Error('status_dict')
                                    ...
                                    if (validatorData){
                                        ...
                                        validator.commission = validatorData.commission;
                                        // console.log(validator);
                                        // throw Error('test valExist');
                                        // calculate self delegation percentage every 30 blocks

                                        if (height % 30 == 1){
                                            let self_delegations_uri = LCD + '/staking/delegators/'+valExist.operator_account+'/delegations/'+valExist.operator_account;
                                            try{
                                                // added_start
                                                // let self_delegations_uri = LCD + '/staking/delegators/'+valExist.operator_account+'/delegations/'+valExist.operator_account;
                                                let response = HTTP.get(self_delegations_uri);
                                                // added_end
                                                ...
                                                console.log(self_delegations_uri);
                                                console.log(e);
                                            }
                                        }
                        ...
                        // console.log('consensus_pubkey_test')
                        // let dk = Meteor.call('bech32ToPubkey', 'kratosvalconspub1zcjduepq9zd65dz7wpxy4vsslf4hmlc7uah5pqpee7y2s67lv3kxdsftn9gqpr9uxh', 'tendermint/PubKeySecp256k1')
                        // let address_2 = getAddress({
