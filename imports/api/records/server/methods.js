    ...
    'ValidatorRecords.calculateMissedBlocks': function(){
        if (!COUNTMISSEDBLOCKS){
            try {
                ...
                // added_start
                // console.log(explorerStatus);
                // console.log(`latestHeight:${latestHeight}, explorerStatus: ${explorerStatus}, startHeight: ${startHeight}, BULKUPDATEMAXSIZE: ${BULKUPDATEMAXSIZE}`)
                // return ;
                // added_end
    ...
    'Analytics.aggregateBlockTimeAndVotingPower': function(time){
        ...
        if (time == 'm'){
            ...
            if (analytics.length > 0){
                ...
                let chain_info = [{chainId:Meteor.settings.public.chainId},{$set:{lastMinuteVotingPower:averageVotingPower, lastMinuteBlockTime:averageBlockTime}}]
                // throw Error('debug 1')
