...
Meteor.methods({
    'chain.getConsensusState': function(){
        ...
        try{
            ...
            let existed_chain_info = Chain.findOne({chainId:Meteor.settings.public.chainId});

            if (existed_chain_info) {
              let n = {$set: {...chain_info}}
              delete n.chainId;
              Chain.update({chainId:Meteor.settings.public.chainId}, n);
            } else {
              Chain.insert(chain_info);
            }
        }
        catch(e){
            console.log(url);
    ...
    'chain.updateStatus': function(){
        ...
        try{
            ...
            // update chain status start
            // chainStates = {}
            // url = LCD + '/supply/total/'+  Meteor.settings.public.bondDenom;
            // console.log(url);
            // console.log('url_supply_total');
            // try{
            //     response = HTTP.get(url);
            ...
            if (parseInt(chain.latestBlockHeight) > 0){
                ...
                console.log('update chain state')
                if ( Meteor.settings.public.bondDenom ) {
                    url = LCD + '/supply/total/'+  Meteor.settings.public.bondDenom;
                    try{
                        response = HTTP.get(url);
                        let supply = JSON.parse(response.content).result;
	    ...
	    if (genesis.app_state.gov) {
            ...
            if (genesis.app_state.genutil && genesis.app_state.genutil.gentxs && (genesis.app_state.genutil.gentxs.length > 0)){
                for (i in genesis.app_state.genutil.gentxs){
                    ...
                    for (m in msg){
                        if (msg[m].type == "kuchain/KuMsgCreateValidator"){
