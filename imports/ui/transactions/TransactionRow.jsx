...
const show_gras = (fee) => {
  let num_str = (new Coin(parseFloat(fee.amount), (fee)?fee.denom:null)).stakeString();
...
export const TransactionRow = (props) => {
    ...
    return <SentryBoundary><Row className={(tx.code)?"tx-info invalid":"tx-info"}>
        <Col xs={12} lg={7} className="activity">{(tx.tx.value.msg && tx.tx.value.msg.length >0)?tx.tx.value.msg.map((msg,i) => {
            return <Card body key={i}><Activities msg={msg} tx_origin={tx.tx_json.msg[i]} invalid={(!!tx.code)} events={(tx.logs&&tx.logs[i])?tx.logs[i].events:null} /></Card>
        }):''}</Col>
        ...
        <Col xs={6} md={9} lg={{size:2,order:"last"}} className="text-nowrap row_align_center"><i className="material-icons">schedule</i> <div>{tx.block()?<TimeAgo time={tx.block().time} />:''}</div>{(tx.tx.value.memo && tx.tx.value.memo != "")?<span>
            <i className="material-icons d-lg-none ml-2 memo-button" id={"memo-"+tx.txhash}>message</i>
        ...
        <Col xs={(!props.blockList)?6:8} md={(!props.blockList)?9:4} lg={2} className="fee"><i className="material-icons d-lg-none">monetization_on</i> {(tx.tx.value.fee.amount.length > 0)?tx.tx.value.fee.amount.map((fee,i) => {
             return <span className="text-nowrap" key={i}> <NumberShow num_str={(new Coin(parseFloat(fee.amount), (fee)?fee.denom:null)).stakeString()}></NumberShow></span>
