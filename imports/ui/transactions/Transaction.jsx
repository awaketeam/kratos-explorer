...
const show_gras = (fee) => {
  let num_str = ((fee.amount/Meteor.settings.public.stakingFraction)>=1)?(new Coin(parseFloat(fee.amount), fee.denom)).stakeString():(new Coin(parseFloat(fee.amount) , fee.denom)).stakeString()
  ...
  return <span className="ft_1222"> {e_arr[0]} x <sup>{ e_arr[1] }</sup> <small> {token_type} </small></span>
    ...
    render(){
        ...
        if (this.props.loading){
            ...
            if (this.props.transactionExist){
                ...
                return <Container id="transaction">
                    <Helmet>
                        <title>Transaction {tx.txhash} on Kratos Explorer | Block-X</title>
                        ...
                        <CardBody>
                            <Row>
                                ...
                                <Col md={8} className="value">{(tx.tx.value.fee.amount.length > 0)?tx.tx.value.fee.amount.map((fee,i) => {
                                    return <span className="text-nowrap" key={i}> <NumberShow num_str={((fee.amount/Meteor.settings.public.stakingFraction)>=1)?(new Coin(parseFloat(fee.amount), fee.denom)).stakeString():(new Coin(parseFloat(fee.amount) , fee.denom)).stakeString()}></NumberShow></span>
                                }):<span><T>transactions.noFee</T></span>}</Col>
                                <Col md={4} className="label">
                                  {
                                    i18n.__('transactions.newgasUsedWanted') || i18n.__('transactions.gasUsedWanted')
                                  }
                                ...
                                <Col md={4} className="label">
                                  {
                                    i18n.__('transactions.newmemo') || i18n.__('transactions.memo')
                                  }
                                </Col>
                    ...
                    {(tx.tx.value.msg && tx.tx.value.msg.length >0)?tx.tx.value.msg.map((msg,i) => {
                        return <Card body key={i}><Activities msg={msg} tx_origin={tx.tx_json.msg[i]} invalid={(!!tx.code)} events={(tx.logs&&tx.logs[i])?tx.logs[i].events:null} denom ={this.denom}/></Card>
                    }):''}
                </Container>
            }
            else{
