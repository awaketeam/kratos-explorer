...
export default class AccountDetails extends Component{
        ...
        Meteor.call('accounts.getBalance', this.props.match.params.address, (error, result) => {
            ...
            if (result){
                ...
                if (result.locks) {
                  ...
                  let locksTotal = 0;
                  let main_tokens_locked = [];
                  for(let lock_item of result.locks){
                    // if(lock_item.denom === Meteor.settings.public.bondDenom ){
                      locksTotal += parseFloat(lock_item.amount)
                      main_tokens_locked.push(lock_item);
                    // }
                  }

                  this.setState({
                    'locks': result.locks,
                    'locksToal': locksTotal,
                    'total': [...main_tokens_locked]
                  });
                ...
                let cp_res = JSON.parse(JSON.stringify(result));
                ...
                if (result.rewards && result.rewards.length > 0){
                    ...
                    for(let c = 0; c < result.rewards.length; c++){
                        if(result.rewards[c].reward != null){
                            numRewards[result.rewards[c]["validator_account"]] = result.rewards[c].reward;
                        }
                    }
    ...
    render(){
        ...
        if (this.state.loading){
            ...
            return <div id="account">
                <Helmet>
                    <title>Account Details of {this.state.address} on Kratos Explorer | Block-X </title>
                            ...
                            <Row className="account-distributions">
                                <Col xs={12}>
                                    <Progress multi>
                                        ...
                                        <Progress bar className="selflock" value={this.state.locksToal/currentCoinTotal*100} />
                                    </Progress>
                                </Col>
                            </Row>
                            <Row>
                                <Col md={6} lg={12}>
                                    <Row className="gray_hover account_asset_row">
                                        <Col xs={9} className="label text-nowrap"><div className="available infinity" /><T>accounts.available</T></Col>
                                        <Col xs={3} className="value text-left">{this.findCoin(this.state.available) || new Coin(0).toString(4) }</Col>
                                    </Row>
                                    <Row className="gray_hover account_asset_row">
                                        <Col xs={9} className="label text-nowrap"><div className="delegated infinity" /><T>accounts.delegated</T></Col>
                                        <Col xs={3} className="value text-left">{new Coin(this.state.delegated).toString(4)}</Col>
                                    </Row>
                                    <Row className="gray_hover account_asset_row"> 
                                        <Col xs={9} className="label text-nowrap"><div className="unbonding infinity" /><T>accounts.unbonding</T></Col>
                                        <Col xs={3} className="value text-left">{new Coin(this.state.unbonding).toString(4)}</Col>
                                    </Row>
                                    <Row className="gray_hover account_asset_row">
                                        <Col xs={9} className="label text-nowrap"><div className="rewards infinity" /><T>accounts.rewards</T></Col>
                                        <Col xs={3} className="value text-left">{this.findCoin(this.state.rewards)  || new Coin(0).toString(4)}</Col>
                                    </Row>
                                    <Row className="gray_hover account_asset_row">
                                        <Col xs={9} className="label text-nowrap"><div className="selflock infinity" />
                                          { i18n.__('accounts.selfLocks') }
                                        </Col>
                                        <Col xs={3} className="value text-left">{ new Coin(this.state.locksToal).toString(4) }</Col>
                                    </Row>
                                    {this.state.commission?<Row className="gray_hover account_asset_row">
                                        <Col xs={9} className="label text-nowrap"><div className="commission infinity" /><T>validators.commission</T></Col>
                                        <Col xs={3} className="value text-left">{this.findCoin(this.state.commission)  || new Coin(0).toString(4)}</Col>
                                    </Row>:null}

                                    <Row className="account_total_asset gray_hover account_asset_row">
                                        <Col xs={9} className="label text-nowrap row_align_center"><T>accounts.total</T></Col>
                                        <Col xs={3} className="value text-left">
                                          <div className="account_coin_ct">
                                            {this.findCoin(this.state.total)}
                                          </div>
                                          <div className="account_coin_ct_small">
                                            {/*~{numbro((this.findValue(this.state.total))/Coin.StakingCoin.fraction*this.state.price).format("$0,0.0000a")} ({numbro(this.state.price).format("$0,0.00")}/{Meteor.settings.public.bondDenomShowAs})*/}
                                          </div>
                                        </Col>
