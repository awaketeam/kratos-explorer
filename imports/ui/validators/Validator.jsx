...
export default class Validator extends Component{
    ...
    getUserDelegations() {
        if (this.state.user && this.props.validator && this.props.validator.address) {
            ...
            // to-to operator_address replacce as operator_account
        ...
        if (this.props.loading){
            ...
            if (this.props.validatorExist){
                ...
                return <Row className="validator-details">
                    <Helmet>
                        <title>{ moniker } - Kratos Explorer | Block-X</title>
                    ...
                    <Card>
                          <div className="card-header">
                            ...
                            { i18n.__('validators.newuptime') ? <T>validators.newuptime</T> : <T>validators.uptime</T> }
                    ...
                    <Col md={8}>
                        ...
                        <CardBody>
                            <Row>
                                ...
                                <Col sm={4} className="label">
                                 { i18n.__('validators.nodeName') || i18n.__('validators.operatorAddress')  }
                                </Col>
                                <Col sm={8} className="value address" data-operator-address={this.props.validator.operator_address}>{this.props.validator.operator_account}</Col>
                        ...
                        <div className="card-header"><T>common.votingPower</T></div>
                            ...
                            <Row>
                                {this.props.validator.voting_power?<Col xs={12}><h1 className="display-4 voting-power"><Badge color="primary" >{numbro(this.props.validator.voting_power).format('0,0')}</Badge></h1></Col>:''}
                                <Col sm={4} className="label">
                                  <T>validators.selfDelegationRatio</T>
                                </Col>
                                <Col sm={8} className="value">
                                  {this.props.validator.self_delegation?<span>{numbro(this.props.validator.self_delegation).format("0,0.00%")}</span>:'N/A'}
                                </Col>

                                <Col sm={4} className="label">
                                  {/*<T>validators.selfDelegationRatio</T>*/}
                                  { i18n.__('validators.selfDelegationAmount') }
                                </Col>
                                <Col sm={8} className="value">
                                  {this.props.validator.self_delegation?<span>{numbro(this.props.validator.voting_power*this.props.validator.self_delegation).format({thousandSeparated: true,mantissa:0})}</span>:'N/A'}
                                </Col>
                                ...
                                <Col sm={4} className="label">
                                  { i18n.__('common.votingPower') }
                                </Col>
                                <Col sm={8} className="value">
                                  {this.props.validator.voting_power?<span>{numbro(this.props.validator.voting_power).format('0,0')}</span>:'N/A'}
                                </Col>

                                <Col sm={4} className="label">
                                  { i18n.__('common.votingPowerPercent') }
                                </Col>
                                <Col sm={8} className="value">
                                  {this.props.validator.voting_power?<span>{numbro(this.props.validator.voting_power/this.props.chainStatus.activeVotingPower).format('0.00%')}</span>:'N/A'}
                                </Col>
                                ...
                                <Row>
                                  <Col md={4} className="label"><T>validators.unbondingHeight</T></Col>
                    ...
                    <Nav pills>
                            <NavItem>
                        <NavLink tag={Link} to={"/validator/"+this.props.validator.operator_account} active={!(this.props.location.pathname.match(/(delegations|transactions)/gm))}><T>validators.powerChange</T></NavLink>
                      </NavItem>
                            <NavItem>
                                <NavLink tag={Link} to={"/validator/"+this.props.validator.operator_account+"/delegations"} active={(this.props.location.pathname.match(/delegations/gm) && this.props.location.pathname.match(/delegations/gm).length > 0)}><T>validators.delegations</T></NavLink>
                      </NavItem>
                            <NavItem>
                                <NavLink tag={Link} to={"/validator/"+this.props.validator.operator_account+"/transactions"} active={(this.props.location.pathname.match(/transactions/gm) && this.props.location.pathname.match(/transactions/gm).length > 0)}><T>validators.transactions</T></NavLink>
                      </NavItem>
                        </Nav>
                        <Switch>
                            <Route exact path="/(validator|validators)/:account" render={() => <div className="power-history">{this.state.history}</div> } />
                            <Route path="/(validator|validators)/:account/delegations" render={() => <ValidatorDelegations account={this.props.validator.operator_account} tokens={this.props.validator.tokens} shares={this.props.validator.delegator_shares} denom={this.props.denom} />} />
                            <Route path="/(validator|validators)/:account/transactions" render={() => <ValidatorTransactions validator={this.props.validator.operator_account} delegator={this.props.validator.operator_account} delegator_account={this.props.validator.delegator_account}  limit={100}/>} />
                      </Switch>
